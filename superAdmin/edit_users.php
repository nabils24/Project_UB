...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($uuid) || empty($id) || empty($username) || empty($role)) {
        ...
        header('Location: listusers.php?warning=Fields cannot be empty');
        exit;
    }
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($conn->query($updateQuery) === TRUE) {
            ...
            header('Location: listusers.php?warning=update_failed');
        }
    } else {
        // Jika tidak ada pengguna ditemukan, redirect dengan pesan error
        header('Location: listusers.php?warning=user_not_found');
    }
} else {
    // Jika bukan permintaan POST, redirect dengan pesan error
    header('Location: listusers.php?error=invalid_request');
}
