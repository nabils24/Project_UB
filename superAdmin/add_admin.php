...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id) || empty($username) || empty($password) || empty($role)) {
        header('Location: listadmin.php?error=empty_fields');
        exit();
    }
    ...
    if ($count > 0) {
        // id already exists
        header('Location: listadmin.php?error=ID already exists');
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        header('Location: listadmin.php?success=admin  added successfully');
    } else {
        // Redirect to the user list page with an error message
        header('Location: listadmin.php?error=Failed to add admin');
    }
